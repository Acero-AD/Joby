    ...
    <div className="flex w-[1440px] h-[900px] mx-auto rounded-[20px] bg-gradient-to-br from-bg-start via-bg-mid1 via-70% to-bg-end">
      ...
      <div className="flex flex-col flex-1 gap-6 p-7 px-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h1 className="text-text-primary text-[26px] font-bold">{position.title || t('table.untitled')}</h1>
            ...
            <div className="flex items-center gap-2.5">
              <button className="flex items-center gap-1.5 rounded-[10px] border border-[#A855F7] py-2 px-4 cursor-pointer hover:bg-purple-50">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="#A855F7" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
              ...
              >
                ...
                <span className="text-[#EF4444] text-sm font-medium">{t('detail.delete')}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[14px] shadow-[0_2px_12px_#A855F712] p-6 flex flex-col gap-5">
          <span className="text-text-primary text-[17px] font-semibold">{t('detail.applicationDetails')}</span>
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-text-muted text-xs font-medium">{t('detail.labels.applicationUrl')}</span>
                {position.url ? (
                  <a
                    href={position.url}
                    target="_blank"
                  ...
                  <span className="text-text-light text-sm">{t('detail.noUrl')}</span>
                )}
              </div>

              {/* Status */}
              <div className="flex flex-col gap-1">
                <span className="text-text-muted text-xs font-medium">{t('detail.labels.status')}</span>
                <div>
                  <StatusBadge status={position.status} />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-text-muted text-xs font-medium">{t('detail.labels.cvFile')}</span>
                {position.cvFilename ? (
                  <div className="flex items-center gap-2.5 bg-[#FAF5FF] border border-[#E9D5FF] rounded-[10px] p-3">
                    ...
                    <div className="flex flex-col gap-0.5 flex-1">
                      <span className="text-[#374151] text-[13px] font-medium">{position.cvFilename}</span>
                    </div>
                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="#A855F7" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" x2="12" y1="15" y2="3" />
                    </svg>
                  </div>
                ) : (
                  <span className="text-text-light text-sm">{t('detail.noCvAttached')}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-5">
              {/* Created */}
              <div className="flex flex-col gap-1">
                <span className="text-text-muted text-xs font-medium">{t('detail.labels.created')}</span>
                <div className="flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="#6B7280" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                    <rect width="18" x="3" y="4" height="18" rx="2" ry="2" /><line x1="16" x2="16" y1="2" y2="6" /><line x1="8" x2="8" y1="2" y2="6" /><line x1="3" x2="21" y1="10" y2="10" />
                  </svg>
                  <span className="text-text-primary text-sm">{position.createdAt}</span>
                </div>
              </div>

              {/* Last Updated */}
              <div className="flex flex-col gap-1">
                <span className="text-text-muted text-xs font-medium">{t('detail.labels.lastUpdated')}</span>
                <div className="flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="#6B7280" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" />
                  </svg>
                  <span className="text-text-primary text-sm">{position.updatedAt}</span>
                </div>
              </div>
            </div>
          </div>
