...
export default function DeleteConfirmationModal({ position, onConfirm, onClose }: DeleteConfirmationModalProps) {
      ...
      >
        ...
        <div className="flex items-center justify-between py-6 px-7 pb-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#FEE2E2] flex items-center justify-center">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="#EF4444" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                <path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3" />
                <path d="M12 9v4" /><path d="M12 17h.01" />
              </svg>
            </div>
            <span className="text-text-primary text-lg font-semibold">{t('deleteModal.title')}</span>
          </div>
          <button
            onClick={onClose}
          ...
          >
            <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="#9CA3AF" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="flex flex-col gap-4 py-5 px-7">
          ...
          <div className="flex items-center gap-3 bg-[#FEF2F2] border border-[#FECACA] rounded-[10px] py-3 px-3.5">
            <svg className="w-[18px] h-[18px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="#EF4444" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
              <path d="M20 7H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2Z" />
              <path d="M16 7V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v3" />
            </svg>
            <div className="flex flex-col gap-0.5">
              <span className="text-text-primary text-sm font-medium">{position.title || t('table.untitled')}</span>
              <span className="text-[#6B7280] text-xs">
                {t('deleteModal.positionContext', { company: position.company || '—', date: position.createdAt })}
              </span>
            </div>
          </div>
