...
The system SHALL display four stat cards in a row: Total Applications, Interviews, Offers, Rejections. Each card SHALL compute its value from the user's positions data.
...
- **THEN** Total Applications SHALL show "10", Interviews SHALL show "2", Offers SHALL show "2", Rejections SHALL show "3"
...
The system SHALL render InterviewItem components inside a dark-themed card (#18181B) showing company name, interview time, type icon, and a status dot.
...
The system SHALL render QuickActionButton components with an icon and label in a light purple-bordered card.

#### Scenario: Quick action buttons displayed
- **WHEN** the dashboard loads
- **THEN** three quick action buttons SHALL be visible: "Upload New CV", "Saved Jobs", "Analytics"
